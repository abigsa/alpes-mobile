...
const PKG = "PKG_LISTA_MATERIALES";

async function insertar(data) {
  const conn = await getConnection();
  try {
    const result = await conn.execute(
      `BEGIN ${PKG}.SP_INSERTAR(:p_producto_id, :p_version, :p_vigencia_inicio, :p_vigencia_fin, :p_estado, :p_id); END;`,
      {
...
async function actualizar(data) {
  ...
  try {
    await conn.execute(
      `BEGIN ${PKG}.SP_ACTUALIZAR(:p_lista_materiales_id, :p_producto_id, :p_version, :p_vigencia_inicio, :p_vigencia_fin, :p_estado); END;`,
      {
