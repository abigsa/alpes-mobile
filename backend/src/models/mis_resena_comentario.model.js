...
// BUSCAR - Filtra reseñas por criterio (cli_id, producto_id, etc)
// NOTA: Usa SP_LISTAR_RESENA_COMENTARIO existente + filtrado en memoria
// Si existe SP_BUSCAR_RESENA_COMENTARIO en Oracle, reemplaza esta función con:
//   const result = await conn.execute(
//     `BEGIN ${PKG}.SP_BUSCAR_RESENA_COMENTARIO(:p_criterio, :p_valor, :p_cursor); END;`,
//     {...}
//   );
async function buscar(criterio, valor) {
  const conn = await getConnection();
  try {
    const allRows = await listar();
    if (criterio && criterio.toLowerCase() === 'cli_id') {
      return allRows.filter(row => (row.CLI_ID || row.cli_id) == valor);
    }
    if (criterio && criterio.toLowerCase() === 'producto_id') {
      return allRows.filter(row => (row.PRODUCTO_ID || row.producto_id) == valor);
    }
    return allRows;
  } finally { await closeConn(conn); }
}
