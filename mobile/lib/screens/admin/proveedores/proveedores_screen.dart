...
class _ProveedoresScreenState extends State<ProveedoresScreen> {
  ...
  void _filtrar() {
    ...
    setState(() {
      _filtrados = q.isEmpty ? List.from(_items) : _items.where((p) {
        final rs  = (p['RAZON_SOCIAL'] ?? p['razon_social'] ?? '').toString().toLowerCase();
        final nit = (p['NIT'] ?? p['nit'] ?? '').toString().toLowerCase();
  ...
  Future<void> _eliminar(dynamic id) async {
    ...
    if (ok != true) return;
    await http.delete(Uri.parse('${ApiConfig.baseUrl}${ApiConfig.proveedores}/$id'));
    _cargar();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(children: [
        Container(color: Colors.white,
          ...
          child: TextField(controller: _searchCtrl, onChanged: (_) => _filtrar(),
            decoration: InputDecoration(hintText: 'Buscar proveedor…',
        ...
        if (!_loading)
          Padding(padding: const EdgeInsets.fromLTRB(16, 10, 16, 0),
            child: Row(children: [
              Text('${_filtrados.length} proveedor${_filtrados.length != 1 ? 'es' : ''}',
                  style: const TextStyle(fontSize: 12, color: AlpesColors.nogalMedio, fontWeight: FontWeight.w500)),
            ])),
        Expanded(
          child: _loading
              ...
              : _filtrados.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.local_shipping_outlined, size: 64, color: AlpesColors.arenaCalida.withOpacity(0.5)),
                      const SizedBox(height: 12),
                      const Text('Sin proveedores', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AlpesColors.nogalMedio)),
                    ]))
                  : RefreshIndicator(color: AlpesColors.cafeOscuro, onRefresh: _cargar,
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          final id  = p['PROV_ID'] ?? p['prov_id'] ?? p['ID'] ?? p['id'];
                          final rs  = p['RAZON_SOCIAL'] ?? p['razon_social'] ?? 'Sin nombre';
                          final nit = p['NIT'] ?? p['nit'] ?? '';
                          final tel = p['TELEFONO'] ?? p['telefono'] ?? '';
                          final email = p['EMAIL'] ?? p['email'] ?? '';
                          final ciudad = p['CIUDAD'] ?? p['ciudad'] ?? '';
                          final initial = rs.isNotEmpty ? rs[0].toUpperCase() : 'P';
                          return Container(
                            ...
                            child: Padding(padding: const EdgeInsets.all(14),
                              child: Row(children: [
                                Container(width: 46, height: 46,
                                    decoration: BoxDecoration(color: AlpesColors.nogalMedio.withOpacity(0.12),
                                        borderRadius: BorderRadius.circular(12)),
                                    alignment: Alignment.center,
                                    child: Text(initial, style: const TextStyle(fontSize: 18,
                                        fontWeight: FontWeight.w700, color: AlpesColors.nogalMedio))),
                                const SizedBox(width: 12),
                                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Text(rs, style: const TextStyle(fontWeight: FontWeight.w700,
                                  ...
                                  Row(children: [
                                    if (tel.isNotEmpty) ...[
                                      ...
                                      Text(tel, style: const TextStyle(fontSize: 11, color: AlpesColors.nogalMedio)),
                                    ],
                                    if (ciudad.isNotEmpty) ...[
                                      ...
                                      Text(ciudad, style: const TextStyle(fontSize: 11, color: AlpesColors.nogalMedio)),
                                    ],
                                  ]),
                                ...
                                Column(children: [
                                  _iBtn(Icons.edit_outlined, AlpesColors.nogalMedio, () => _abrirForm(p)),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Nuevo proveedor', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
...
class _ProveedoresForm extends StatefulWidget {
  ...
  const _ProveedoresForm({this.item, required this.onGuardado});
  @override State<_ProveedoresForm> createState() => __ProveedoresFormState();
}
...
class __ProveedoresFormState extends State<_ProveedoresForm> {
  ...
  void initState() {
    super.initState();
    _c = { 'razon_social': TextEditingController(), 'nit': TextEditingController(),
      'email': TextEditingController(), 'telefono': TextEditingController(),
      'direccion': TextEditingController(), 'ciudad': TextEditingController(),
  ...
  Future<void> _guardar() async {
    ...
    try {
      ...
      final idKey = widget.item?.keys.firstWhere((k) => k.toLowerCase() == 'prov_id', orElse: () => '') ?? '';
      final id = idKey.isNotEmpty ? widget.item![idKey] : null;
      final uri = Uri.parse('${ApiConfig.baseUrl}${ApiConfig.proveedores}${id != null ? '/$id' : ''}');
      final res = id != null
  ...
  Widget _f(String label, String key, {TextInputType? type, bool req = false}) => Padding(
      ...
      child: Container(padding: const EdgeInsets.fromLTRB(20, 8, 20, 20),
          child: Form(key: _fk, child: SingleChildScrollView(child: Column(
              ...
              children: [
                ...
                Text(widget.item == null ? 'Nuevo proveedor' : 'Editar proveedor',
