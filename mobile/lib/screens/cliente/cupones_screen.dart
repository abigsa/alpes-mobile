...
class _CuponesScreenState extends State<CuponesScreen> {
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        trailing: Column(
          ...
          children: [
            Text(
              '${cupon['valor_descuento']}${cupon['tipo_descuento'] == 'porcentaje' ? '%' : '\$'}',
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: _fila('Uso total', '${cupon['limite_uso_total']} usos'),
                    ),
                    Expanded(
                      child: _fila('Por cliente', '${cupon['limite_uso_por_cliente']} usos'),
                    ),
                  ],
                ),
                const Divider(),
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed: () => _confirmarEliminar(context, cupon['cupon_id']),
  ...
  bool _esCuponActivo(Map<String, dynamic> cupon) {
    try {
      ...
      final inicio = DateTime.parse(cupon['vigencia_inicio']?.toString() ?? '');
      final fin = DateTime.parse(cupon['vigencia_fin']?.toString() ?? '');
      final usosRestantes = (cupon['limite_uso_total'] ?? 0) - (cupon['usos_actuales'] ?? 0);
          ...
          if (mounted) {
            if (resultado) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Cupón creado exitosamente'), backgroundColor: Colors.green),
              );
              Navigator.pop(context);
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(provider.mensajeCupon ?? 'Error'), backgroundColor: Colors.red),
              );
            }
          }
        },
      ...
      builder: (context) => _DialogoFormularioCupon(
        ...
        onGuardar: (datos) async {
          ...
          if (mounted) {
            if (resultado) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Cupón actualizado'), backgroundColor: Colors.green),
              );
              Navigator.pop(context);
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (mounted) {
                ...
                if (resultado) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Cupón eliminado'), backgroundColor: Colors.green),
                  );
                }
              }
            },
...
class _DialogoFormularioCupon extends StatefulWidget {
  ...
  @override
  State<_DialogoFormularioCupon> createState() => _DialogoFormularioCuponState();
}
...
class _DialogoFormularioCuponState extends State<_DialogoFormularioCupon> {
  ...
  void initState() {
    super.initState();
    _codigoCtrl = TextEditingController(text: widget.cuponActual?['codigo'] ?? '');
    _descripcionCtrl = TextEditingController(text: widget.cuponActual?['descripcion'] ?? '');
    _valorCtrl = TextEditingController(text: widget.cuponActual?['valor_descuento']?.toString() ?? '');
    _limiteUsoCtrl = TextEditingController(text: widget.cuponActual?['limite_uso_total']?.toString() ?? '100');
    _limiteClienteCtrl = TextEditingController(text: widget.cuponActual?['limite_uso_por_cliente']?.toString() ?? '1');
    _tipoDescuento = widget.cuponActual?['tipo_descuento'] ?? 'porcentaje';
    
    try {
      _vigenciaInicio = widget.cuponActual != null ? DateTime.parse(widget.cuponActual!['vigencia_inicio'].toString()) : DateTime.now();
      _vigenciaFin = widget.cuponActual != null ? DateTime.parse(widget.cuponActual!['vigencia_fin'].toString()) : DateTime.now().add(const Duration(days: 30));
    } catch (e) {
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            TextField(controller: _codigoCtrl, decoration: const InputDecoration(labelText: 'Código', hintText: 'DESCUENTO50')),
              ...
              children: [
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    onChanged: (val) => setState(() => _tipoDescuento = val ?? 'porcentaje'),
                    decoration: const InputDecoration(labelText: 'Tipo'),
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: TextField(controller: _valorCtrl, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: _tipoDescuento == 'porcentaje' ? 'Valor (%)' : 'Valor (\$)')),
                ),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(child: TextField(controller: _limiteUsoCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Límite uso total'))),
                const SizedBox(width: 12),
                Expanded(child: TextField(controller: _limiteClienteCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Por cliente'))),
              ],
            ),
            const SizedBox(height: 12),
            _selectorfecha('Inicio', _vigenciaInicio, (f) => setState(() => _vigenciaInicio = f)),
            const SizedBox(height: 8),
            _selectorfecha('Fin', _vigenciaFin, (f) => setState(() => _vigenciaFin = f)),
          ],
        ),
      ),
      actions: [
  ...
  Widget _selectorfecha(String label, DateTime fecha, Function(DateTime) onChanged) {
    return GestureDetector(
      onTap: () async {
        final picked = await showDatePicker(context: context, initialDate: fecha, firstDate: DateTime(2020), lastDate: DateTime(2030));
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(DateFormat('dd/MM/yyyy').format(fecha), style: const TextStyle(fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
  ...
  void _guardar() {
    if (_codigoCtrl.text.isEmpty || _descripcionCtrl.text.isEmpty || _valorCtrl.text.isEmpty) {
