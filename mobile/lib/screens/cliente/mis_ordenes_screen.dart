...
class _MisOrdenesScreenState extends State<MisOrdenesScreen> {
  ...
  final _filtros = ['Todos', 'Pendiente', 'En proceso', 'Entregado', 'Cancelado'];
  ...
  Future<void> _cargar() async {
    ...
    try {
      ...
      if (data['ok'] == true) {
        setState(() => _ordenes = List<Map<String, dynamic>>.from(data['data']));
      }
    } catch (_) {}
  ...
  IconData _iconEstado(String e) {
    switch (e.toLowerCase()) {
      ...
      case 'en proceso': return Icons.local_shipping_rounded;
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: active
                          ? AlpesColors.cafeOscuro : AlpesColors.arenaCalida),
                    ),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Container(
                ...
                child: Icon(_iconEstado(estado), color: _colorEstado(estado), size: 20),
              ),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Orden $num', style: const TextStyle(fontSize: 14,
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                ...
                Container(
                  ...
                  child: Text(estado, style: TextStyle(fontSize: 10,
                      fontWeight: FontWeight.w700, color: _colorEstado(estado))),
                ),
              ]),
            ]),
  ...
  Widget _buildMiniTimeline(String estado) {
    ...
    int currentIdx = pasos.indexWhere(
        (p) => p.toLowerCase() == estado.toLowerCase());
    if (currentIdx == -1) currentIdx = estado.toLowerCase() == 'cancelado' ? -1 : 0;
    ...
    if (estado.toLowerCase() == 'cancelado') {
      return Container(
        ...
        child: const Row(children: [
          ...
          SizedBox(width: 6),
          Text('Orden cancelada', style: TextStyle(fontSize: 11,
              color: AlpesColors.rojoColonial, fontWeight: FontWeight.w600)),
        ]),
      );
    }

    return Row(children: List.generate(pasos.length * 2 - 1, (i) {
      if (i.isOdd) {
      ...
      return Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(pasos[idx], style: TextStyle(fontSize: 8.5, fontWeight: FontWeight.w500,
            color: done ? AlpesColors.cafeOscuro : AlpesColors.arenaCalida)),
      ]);
    }));
  }
  ...
  Widget _emptyState() => Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(
        ...
        child: const Icon(Icons.receipt_long_outlined, size: 44,
            color: AlpesColors.arenaCalida),
      ),
      const SizedBox(height: 16),
      const Text('Sin órdenes', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700,
          color: AlpesColors.cafeOscuro)),
      const SizedBox(height: 6),
      Text(_filtro == 'Todos' ? 'Aún no has realizado ningún pedido'
          : 'No tienes órdenes con estado "$_filtro"',
          style: const TextStyle(fontSize: 13, color: AlpesColors.nogalMedio)),
