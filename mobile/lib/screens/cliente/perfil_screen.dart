...
class _PerfilScreenState extends State<PerfilScreen> {
  ...
  Future<void> _cargarStats() async {
    ...
    int pedidos = 0, favoritos = 0, resenas = 0;
    ...
    try {
      ...
      if (data['ok'] == true) {
        pedidos = (data['data'] as List).length;
      }
    } catch (_) {}

    // Favoritos reales del cliente
    try {
      final res = await http.get(Uri.parse(
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(children: [
                // ── Stats reales ──
                Row(children: [
                  _statCard('Pedidos', _loadingStats ? '…' : '$_pedidos',
                      Icons.shopping_bag_rounded, context),
                  const SizedBox(width: 10),
                  _statCard('Favoritos', _loadingStats ? '…' : '$_favoritos',
                      Icons.favorite_rounded, context),
                  const SizedBox(width: 10),
                  _statCard('Reseñas', _loadingStats ? '…' : '$_resenas',
                      Icons.star_rounded, context),
                ]),
                ...
                _menuCard([
                  _MenuItem('Mis órdenes', Icons.receipt_long_rounded,
                      () => context.go('/mis-ordenes')),
                  _MenuItem('Mis favoritos', Icons.favorite_rounded,
                      () => context.go('/favoritos')),
                  _MenuItem('Mis tarjetas', Icons.credit_card_rounded,
                      () => context.go('/mis-tarjetas')),
                  _MenuItem('Soporte', Icons.headset_mic_rounded,
                      () => context.go('/soporte')),
                ]),
                ...
                _infoCard([
                  ...
                  _InfoRow(Icons.email_rounded, 'Correo',
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: AlpesColors.rojoColonial.withOpacity(0.15)),
                  ),
                  child: ListTile(
                    leading: Container(
                      width: 40,
  ...
  Widget _statCard(
      String label, String value, IconData icon, BuildContext ctx) {
    return Expanded(
        child: Container(
      ...
      child: Column(children: [
        ...
        Text(label,
            style:
                const TextStyle(fontSize: 10, color: AlpesColors.nogalMedio)),
      ]),
    ));
  }
